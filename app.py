...
# --- DOWNLOAD BUTTON ---
if col3.button("💾 **Download**", use_container_width=True) and st.session_state.rewritten_text:
    st.download_button(
...
if st.session_state.rewritten_text:
    ...
    <div class="glass-box" style="border: 2px solid rgba(0, 255, 255, 0.3); margin-top: 2rem;">
        <h3 style="color: #00eaff; text-align: center; margin-bottom: 1.5rem;">
            ✨ Rewritten Text • Similarity: {st.session_state.last_similarity:.1f}%
        </h3>
    </div>
    """, unsafe_allow_html=True)
    
    # Display rewritten text in a nice container
    st.markdown(f"""
    <div style="
    ...
    # Success message based on similarity
    if st.session_state.last_similarity <= 15:
        st.balloons()
        st.success(f"🎉 **Excellent!** Achieved {st.session_state.last_similarity:.1f}% similarity - Well below target!")
    elif st.session_state.last_similarity <= target_similarity:
        st.success(f"✅ **Success!** Achieved {st.session_state.last_similarity:.1f}% similarity - Within target range!")
    else:
        st.warning(f"⚠️ **Moderate Success** - {st.session_state.last_similarity:.1f}% similarity. Try increasing attempts for better results.")
...
<div class="footer">
    ...
    📚 Optimized for Academic and Research Text
</div>
""", unsafe_allow_html=True)